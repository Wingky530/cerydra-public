...
async function scrapeAnichin(titles: string[], episode: string): Promise<ScrapedSource[]> {
  ...
  for (const r of searchResults) {
    ...
    $('.eplister ul li a').each((i, el) => {
      ...
      const u = $(el).attr('href');
      if (epNum && u) episodes.push({ epText: epNum, url: u });
    });
    
    let targetEp = episodes.find(ep => parseInt(ep.epText, 10) === parseInt(episode, 10));
    if (!targetEp) return [];
    
    const epRes = await fetchWithRetry(`${proxyUrl}${encodeURIComponent(targetEp.url)}`, {
      headers: { 'User-Agent': 'Cerydra-Backend/1.0' }
    });
    if (!epRes.ok) return [];
    const epHtml = await epRes.text();
    ...
    for (const opt of options) {
      try {
        const optRes = await fetchWithRetry(`${proxyUrl}${encodeURIComponent(opt.url)}`, {
          headers: { 'User-Agent': 'Cerydra-Backend/1.0' }
        });
        ...
        let iframeUrl = '';
        $opt('iframe').each((i: any, el: any) => {
          const src = $opt(el).attr('data-litespeed-src') || $opt(el).attr('data-src') || $opt(el).attr('src');
          if (src && src !== 'about:blank' && !iframeUrl) iframeUrl = src;
        });
        if (iframeUrl) {
          if (iframeUrl.startsWith('//')) iframeUrl = 'https:' + iframeUrl;
          let sourceName = opt.name.replace(/\\n|\n/g, '').trim();
          if (!sourceName || sourceName.toLowerCase().includes('select')) sourceName = 'Anichin';
          results.push({
        ...
        console.error('[Anichin] Failed to extract from option', opt.url, e);
      }
    }
...
// MEGAPLAY RESOLVER
// ==========================================
export async function resolveMegaPlay(anilistId: string, episode: string, mode: 'sub' | 'dub'): Promise<any> {
  const url = `https://megaplay.buzz/stream/ani/${anilistId}/${episode}/${mode}`;
  return {
    sourceName: 'MegaPlay',
    type: 'iframe',
    decodedPath: url,
...
export const GET: APIRoute = async ({ request, clientAddress }) => {
  ...
  const clientIp = request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip') || clientAddress || '';
  ...
  const cacheKey = `episode-links:v5:${id}:${episode}:${mode}:${anilistId || 'none'}`;
  if (!nocache) {
  ...
  try {
    ...
    // if (anilistId) {
      ...
      megaPlayPromise = resolveMegaPlay(anilistId, episode, megaPlayMode, clientIp).catch(err => {
    ...
    if (mode === 'sub-id' && title) {
      ...
      try {
        const [otakudesuScraped, anichinScraped] = await Promise.all([
          ...
          scrapeAnichin([englishTitle || '', title, romajiTitle || ''], episode).catch(e => {
            console.error('[Anichin Scraper Error]', e);
            return [] as ScrapedSource[];
          })
        ]);
        
        const scraped = [...otakudesuScraped, ...anichinScraped];
